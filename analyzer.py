...
class BISTAnalyzer:
    ...
    def kontrol_temel_veriler(self, hisse: Dict) -> tuple[bool, Dict]:
        ...
        if not borç_ok and borç > 0:
            geçti = False
        
        # ADV
        adv = hisse.get('adv', 0)
        adv_ok = adv >= FILTER_RULES['min_adv_milyon']
    ...
    def kontrol_teknik(self, hisse: Dict) -> tuple[bool, Dict]:
        ...
        if not adx_ok and adx > 0:
            geçti = False
        
        # Volatilite (ATR)
        atr = hisse.get('atr', 0)
        fiyat = hisse.get('fiyat', 1)
        volatilite = (atr / fiyat * 100) if fiyat > 0 else 0
    ...
    def kontrol_uyarılar(self, hisse: Dict) -> Optional[str]:
        ...
        # Düşük Likidite
        adv = hisse.get('adv', 0)
        if 0 < adv < FILTER_RULES['min_adv_milyon'] / 2:
            return f"Düşük Likidite ({adv:.0f}M) - Kaymayı Kontrol Et"
        
        # Yüksek P/E
        pe = hisse.get('pe', 0)
        if pe > FILTER_RULES['max_pe_ratio'] * 1.5:
    ...
    def analiz_hisse(self, hisse: Dict) -> Dict:
        ...
        tarama_geçti = skor >= 70 and teknik_ok
        ...
        return {
            ...
            'temel_ok': temel_ok,
            'teknik_ok': teknik_ok,
            'giriş_noktaları': giriş_noktaları,
            'uyarı': uyarı,
